...
export class AccountComponent implements OnInit {
  ...
  getAccountInfo(){
    let params = {};
    this.httpService.get('api/profile',params).subscribe(res => {
      if (res.result) {
          let item = res.result; 
  ...
  saveAccountInfo(){
    ...
    if (this.isInValid(this.accountInfo.fullName) ||
    ...
    this.httpService.put('api/profile',params).subscribe(res => {
      if (res) {
  ...
  chnagePassword(){
    ...
    if (this.isInValid(this.changePassword.password) ||
    ...
    if (this.changePassword.newPassword.length < 6 || this.changePassword.confirmPassword.length < 6) {
      this.isError1 = true;
      return;
    }
    ...
    this.httpService.post('api/change-password',params).subscribe(res => {
      if (res) {
  ...
  saveDeliveryAddress(){
    ...
    this.httpService.post('api/delivery-address',params).subscribe(res => {
      if (res) {
  ...
  saveBillingAddress(){
    ...
    this.httpService.post('api/billing-address',params).subscribe(res => {
      if (res) {
